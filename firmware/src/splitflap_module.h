...
inline void SplitflapModule::GoToTargetFlapIndex() {
    ...
    if (target_step == current_step && current_accel_step == 0) {
        // Already at target and stopped — no movement needed.
        // (The old `<=` comparison triggered a spurious full revolution here
        // because minimum_stopping_step == current_step when accel == 0.)
        delta_steps = 0;
    } else if (target_step <= minimum_stopping_step) {
        // Must go around — can't stop in time
